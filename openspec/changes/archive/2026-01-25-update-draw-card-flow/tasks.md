## 1. UI & State Logic
- [x] 1.1 Update `App.tsx` / `store.ts` to introduce a `isConfirmingDraw` state.
    - Triggered when clicking the Draw Pile.
- [x] 1.2 Update `DrawCardOverlay` to strictly separate "Confirmation" vs "Reveal" modes.
    - Ensure `overflow-hidden` is removed or adjusted so the card is not clipped.
- [x] 1.3 Implement the "Confirm" handler.
    - Calls the actual `drawFromDeck` action.
    - Transitions Overlay to "Reveal" mode.

## 2. Animation & Polish
- [x] 2.1 Implement the "Reveal" timer.
    - Keep card visible for ~2 seconds.
- [x] 2.2 Implement "Fly to Hand" animation.
